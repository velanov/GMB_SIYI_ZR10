...
class CameraStreamTkinter:
    """
    Alternative implementation for Tkinter (if needed)
    """
    def __init__(self, gimbal_ip: str = "192.168.144.25", stream_port: int = 8554):
        self.gimbal_ip = gimbal_ip
        self.stream_port = stream_port
        self.rtsp_url = f"rtsp://{gimbal_ip}:{stream_port}/main.264"
    ...
    def start_stream(self, frame_callback: Callable = None):
        """Start camera stream with callback for new frames"""
        if self._running:
            return
    ...
    def _capture_loop(self):
        """Capture loop for Tkinter version"""
        try:
            ...
            while self._running:
                ret, frame = self.cap.read()
                if ret and frame is not None and self.frame_callback:
            ...
            print(f"[CAMERA] Tkinter stream error: {e}")
        finally:
            if self.cap:
                self.cap.release()